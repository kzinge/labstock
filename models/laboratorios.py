...
class Lab(db.Model):
    ...
    materias = db.relationship('Material', backref= 'lab', lazy = True)
...
class ReservaLaboratorio(db.Model):
    ...
    laboratorio = db.relationship('Lab', backref='reservas_laboratorios', lazy=True)
    usuario = db.relationship('User', backref='reservas_laboratorios', lazy=True)
